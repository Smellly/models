...
def _load_and_process_metadata(captions_file, image_dir):
  ...
  id_to_filename = [(x["imgid"], x["filename"]) for x in caption_data["images"]]

  # Extract the captions. Each image_id is associated with multiple captions.
  id_to_captions = {}
  for item in caption_data["images"]:
    image_id = item["imgid"]
    id_to_captions.setdefault(image_id, [])
    for annotation in item["sentences"]:
      caption = annotation["raw"]
...
def main(unused_argv):
  ...
  # flickr30_val_dataset = _load_and_process_metadata(FLAGS.val_captions_file,
  #                                                 FLAGS.val_image_dir)
  ...
  train_cutoff = int(0.10 * len(flickr30_train_dataset))
  train_dataset = flickr30_train_dataset[train_cutoff*2:]
  val_dataset = flickr30_train_dataset[0:train_cutoff]
  test_dataset = flickr30_train_dataset[train_cutoff:train_cutoff*2]
